...
' ------------------------------------------------------------------------------
	Private Sub AddField(ByRef pstrName, ByRef pstrFileName, ByRef pstrContentType, ByRef pstrValue, ByRef pbinData)
		...
		'LF - Verifica se já existe o campo
		Dim llngIndex	' Index of current field
		for llngIndex = 0 to ubound(mobjFieldAry)
			if mobjFieldAry(llngIndex).Name = pstrName then 
				if len(trim(pstrValue)) > 0 then mobjFieldAry(llngIndex).value = mobjFieldAry(llngIndex).value  & ", " & pstrValue
				exit Sub
			end if
		next
